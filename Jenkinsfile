pipeline {
    ...
    parameters {
        string(name: 'DOCKER_IMAGE_NAME', description: 'Enter the Docker image name', defaultValue: 'raghu021/custom-img-17-04-2024')
    }
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'dockerhub_id', variable: 'DOCKERHUB_CREDENTIALS_PSW')]) {
                        sh "echo \${DOCKERHUB_CREDENTIALS_PSW} | sudo docker login -u raghu021 --password-stdin"
                    }
                }
            }
        }
